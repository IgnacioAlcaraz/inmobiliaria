...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const q = supabase
      .from("trackeo_diario")
      .select(
        "fecha, llamadas, captaciones, captaciones_valor, reservas_puntas, reservas_valor_oferta, cierres_operaciones_puntas, cierres_honorarios",
      )
    ...
    if (error)
      return managerError("Error al consultar trackeo: " + error.message, 500);
    ...
    for (const r of data || []) {
      ...
      months[key].cierres_operaciones_puntas += Number(
        r.cierres_operaciones_puntas || 0,
      );
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      reservas_by_month.push({
        month: i,
        reservas_puntas: v.reservas_puntas,
        reservas_valor_oferta: v.reservas_valor_oferta,
      });
      cierres_by_month.push({
        month: i,
        cierres_puntas: v.cierres_operaciones_puntas,
        cierres_honorarios: v.cierres_honorarios,
      });
      captaciones_by_month.push({
        month: i,
        captaciones: v.captaciones,
        captaciones_valor: v.captaciones_valor,
      });
    }

    return managerSuccess(
      { reservas_by_month, cierres_by_month, captaciones_by_month },
      3,
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("trackeo-reservas-vs-cierres error:", msg);
    return managerError(
