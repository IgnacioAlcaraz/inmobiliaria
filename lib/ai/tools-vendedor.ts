  ...
  return [
    ...
    tool(
      async ({ anio, mes, fechaDesde, fechaHasta }) =>
        callRoute('/api/agent/cierres', { userId, anio, mes, fechaDesde, fechaHasta }),
      {
        ...
        schema: z.object({
          ...
          mes: z.number().min(1).max(12).optional().describe('Mes (1-12). Ej: 2 para febrero'),
          fechaDesde: z.string().optional().describe('Fecha inicio en formato YYYY-MM-DD'),
          fechaHasta: z.string().optional().describe('Fecha fin en formato YYYY-MM-DD'),
        }),
      }
    ),

    tool(
      async ({ operacion, conCierre, sinCierre, diasHastaVencimiento, limit }) =>
        callRoute('/api/agent/captaciones', { userId, operacion, conCierre, sinCierre, diasHastaVencimiento, limit }),
      {
        ...
        schema: z.object({
          operacion: z.enum(['Venta', 'Alquiler', 'Temporario']).optional().describe('Tipo de operación'),
          conCierre: z.boolean().optional().describe('true = solo captaciones cerradas'),
          sinCierre: z.boolean().optional().describe('true = solo captaciones activas (sin fecha de cierre)'),
          diasHastaVencimiento: z.number().optional().describe('Filtrar captaciones que vencen en los próximos N días. Ej: 15'),
          limit: z.number().optional().describe('Cantidad máxima de resultados. Default 200'),
        }),
      }
    ),
    ...
    tool(
      async ({ anio, mes, fechaDesde, fechaHasta }) =>
        callRoute('/api/agent/trackeo', { userId, anio, mes, fechaDesde, fechaHasta }),
      {
    ...
    tool(
      async ({ estado, tipo_cliente, prioridad, pendientes, vencidos, limit }) =>
        callRoute('/api/agent/contactos', { userId, estado, tipo_cliente, prioridad, pendientes, vencidos, limit }),
      {
        name: 'get_contactos_usuario',
        description:
          'Lista de contactos/clientes con estado, tipo, seguimiento y próximos pasos. Usar cuando el usuario pregunta por contactos, clientes, seguimientos pendientes, leads o pipeline de clientes.',
        schema: z.object({
          ...
          pendientes: z.boolean().optional().describe('true = solo seguimientos no realizados'),
          vencidos: z.boolean().optional().describe('true = seguimientos pendientes con fecha vencida'),
          limit: z.number().optional().describe('Cantidad máxima de resultados. Default 100'),
        }),
      }
    ),
  ]
