...
export function ManagerCaptacionesVsOperaciones({ captacionesBusquedas, vendedores, year }: Props) {
  const [page, setPage] = useState(0)

  // Build vendedor name lookup
  const vendedorMap = useMemo(() => {
    const m: Record<string, string> = {}
    vendedores.forEach((v) => { m[v.id] = v.full_name || 'Sin nombre' })
    return m
  }, [vendedores])

  // Sort: by fecha_alta descending
  const sorted = useMemo(() =>
    [...captacionesBusquedas].sort((a, b) =>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <h2 className="text-base font-semibold text-muted-foreground mt-1">
          CAPTACIONES Y BÚSQUEDAS VS OPERACIONES CERRADAS AÑO {year}
        </h2>
        <p className="text-xs text-muted-foreground mt-1">
          {sorted.length} registros totales — Página {page + 1} de {totalPages}
        </p>
      </div>
        ...
        <Table className="text-xs">
          <TableHeader>
            <TableRow className="bg-muted/30">
              <TableHead className={`${hClass} sticky left-0 bg-muted/30 z-10`}>Agente</TableHead>
          ...
          <TableBody>
            {pageData.length === 0 ? (
              ...
              pageData.map((c) => {
                ...
                const propiedad = [c.direccion, c.barrio].filter(Boolean).join(', ') || '-'
                ...
                return (
                  <TableRow key={c.id} className={rowClass}>
                    <TableCell className={`${cClass} font-medium sticky left-0 bg-background z-10`}>
                      {vendedorMap[c.user_id] || '-'}
                    </TableCell>
                    <TableCell className={cClass}>{c.codigo_id || c.id?.slice?.(0, 8) || '-'}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_alta)}</TableCell>
                    <TableCell className={cClass}>{c.autorizacion || '-'}</TableCell>
                    <TableCell className={`${cClass} max-w-[200px] truncate`} title={propiedad}>{propiedad}</TableCell>
                    <TableCell className={cClass}>{c.ciudad || '-'}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.vence)}</TableCell>
                    <TableCell className={cClass}>{c.adenda || '-'}</TableCell>
                    <TableCell className={cClass}>{c.operacion || '-'}</TableCell>
                    <TableCell className={`${cClass} text-right`}>{pub > 0 ? formatCurrency(pub) : '-'}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_baja)}</TableCell>
                    <TableCell className={cClass}>{monedaSymbol(c.moneda)}</TableCell>
                    <TableCell className={`${cClass} text-right`}>{oferta > 0 ? formatCurrency(oferta) : '-'}</TableCell>
                    ...
                    <TableCell className={cClass}>{fmtDate(c.fecha_reserva)}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_aceptacion)}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_notificacion)}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_refuerzo)}</TableCell>
                    <TableCell className={cClass}>{fmtDate(c.fecha_cierre)}</TableCell>
                    <TableCell className={`${cClass} text-right font-medium`}>{honTot > 0 ? formatCurrency(honTot) : '-'}</TableCell>
                    <TableCell className={`${cClass} text-right`}>{honAgente}</TableCell>
                    <TableCell className={`${cClass} max-w-[180px] truncate`} title={c.observaciones || ''}>
                      {c.observaciones || '-'}
                    </TableCell>
                  </TableRow>
                )
