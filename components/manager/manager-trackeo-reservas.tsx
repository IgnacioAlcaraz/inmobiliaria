...
export function ManagerTrackeoGlobal({
  ...
  const rowsByMonth = Array.from({ length: 12 }, (_, i) => {
    ...
    const reservasPuntas = rows.reduce(
      (s, r) => s + Number(r.reservas_puntas || 0),
      0,
    );
    const reservasValor = rows.reduce(
      (s, r) => s + Number(r.reservas_valor_oferta || 0),
      0,
    );
    const cierresPuntas = rows.reduce(
      (s, r) => s + Number(r.cierres_operaciones_puntas || 0),
      0,
    );
    const cierresHonorarios = rows.reduce(
      (s, r) => s + Number(r.cierres_honorarios || 0),
      0,
    );
    const llamadas = rows.reduce((s, r) => s + Number(r.llamadas || 0), 0);
    const visitas = rows.reduce((s, r) => s + Number(r.visitas || 0), 0);
    const captBusMes = (captacionesBusquedas || []).filter(
      (c) => new Date(c.fecha_alta).getMonth() === i,
    );
    const captacionesCnt = captBusMes.length;
    const captacionesValor = captBusMes.reduce(
      (s, c) => s + Number(c.valor_publicado || 0),
      0,
    );
  ...
  return (
    <div className="flex flex-col gap-6 page-enter">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 stagger-children">
        ...
        />
        <KpiCard
          title="Reservas (puntas)"
          value={String(totalReservasPuntas)}
          icon={Handshake}
          variant="success"
        />
        <KpiCard
          title="Cierres (puntas)"
          value={String(totalCierresPuntas)}
          icon={TrendingUp}
          variant="accent"
        />
        <KpiCard title="Llamadas" value={String(totalLlamadas)} icon={Phone} />
      </div>
              ...
              <Table>
                ...
                <TableBody>
                  {rowsByMonth.map((r) => {
                    ...
                    return (
                      <TableRow key={r.monthIndex}>
                        ...
                        <TableCell>
                          {manualMap[
                            `reservas_vs_cierres::${r.monthIndex}::puntas_reservas_inicio_mes`
                          ]?.value ?? r.reservasPuntas}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `reservas_vs_cierres::${r.monthIndex}::reservas_honorarios_inicio_mes`
                          ]?.value ?? formatCurrency(r.reservasValor)}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `reservas_vs_cierres::${r.monthIndex}::cantidad_cierres_puntas`
                          ]?.value ?? r.cierresPuntas}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `reservas_vs_cierres::${r.monthIndex}::honorarios_brutos_por_cierres`
                          ]?.value ?? formatCurrency(r.cierresHonorarios)}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      {rowsByMonth.reduce((s, r) => s + r.reservasPuntas, 0)}
                    </TableCell>
                    <TableCell>
                      {formatCurrency(
                        rowsByMonth.reduce((s, r) => s + r.reservasValor, 0),
                      )}
                    </TableCell>
                    <TableCell>
                      {rowsByMonth.reduce((s, r) => s + r.cierresPuntas, 0)}
                    </TableCell>
                    <TableCell>
                      {formatCurrency(
                        rowsByMonth.reduce(
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Trackeo Numérico</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            <div className="border rounded-lg overflow-x-auto">
                  ...
                  {rowsByMonth.map((r) => {
                    ...
                    return (
                      <TableRow key={r.monthIndex}>
                        ...
                        <TableCell>
                          {manualMap[
                            `numerico::${r.monthIndex}::regalias_master`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `numerico::${r.monthIndex}::comision_director`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `numerico::${r.monthIndex}::honorarios_martillero`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>{comAgentes || "-"}</TableCell>
                        <TableCell>
                          {formatCurrency(honorarios - (comAgentes || 0))}
                        </TableCell>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Trackeo Capital Humano</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            <div className="border rounded-lg overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {rowsByMonth.map((r, idx) => {
                    ...
                    const totalAgents = (vendedores || []).filter(
                    ...
                    return (
                      <TableRow key={r.monthIndex}>
                        ...
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_inicio_mes`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_nuevos`
                          ]?.value ?? nuevos}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::bajas_agentes`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::total_agentes`
                          ]?.value ?? totalAgents}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_principiantes`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_buscadores`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_encaminados`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_logradores`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_referentes`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `capital_humano::${r.monthIndex}::agentes_desvinc`
                          ]?.value ?? "-"}
                        </TableCell>
                      </TableRow>
                    );
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Trackeo Evolución Cartera</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            <div className="border rounded-lg overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {rowsByMonth.map((r) => {
                    ...
                    const ticketProm =
                      r.captacionesCnt > 0
                        ? Math.round(r.captacionesValor / r.captacionesCnt)
                        : null;
                    return (
                      <TableRow key={r.monthIndex}>
                        <TableCell>{r.monthName}</TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::cartera_inicio_mes`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::nuevas_captaciones_venta`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::nuevas_captaciones_alquiler`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>{r.reservasPuntas}</TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::dadas_de_baja`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>{totalCartera}</TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::exclusivas`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {manualMap[
                            `evolucion_cartera::${r.monthIndex}::pct_exclusividad`
                          ]?.value ?? "-"}
                        </TableCell>
                        <TableCell>
                          {ticketProm ? formatCurrency(ticketProm) : "-"}
                        </TableCell>
                      </TableRow>
                    );
                  })}
